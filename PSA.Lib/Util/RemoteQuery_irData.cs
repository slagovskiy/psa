...
namespace PSA.Lib.Util
{
	...
	{
		private void QueryData_ir(string kiosk, string order)
		{
			...
			while ((!ok) && (take > 0))
			{
				try
				{
					string Url = "http://print.fotoland.ru/photo/psa.get.irorder.php?k=" + kiosk + "&o=" + order;
					XmlTextReader xmlTextReader = new XmlTextReader(Url);
					xml.Load(xmlTextReader);
					xmlTextReader.Close();
					XmlNode root = xml.GetElementsByTagName("IRORDER")[0];
					foreach (XmlNode node in root.ChildNodes)
					{
						if (node.Name == "HEADER")
						{
							...
							head.Add("CUSTOMER", ((t_customer == "") ? "Заказ через интернет" : t_customer));
							...
							m += "Заказ: " + t_number + ", принят " + t_stamp + " Клиент: " + ((t_customer == "") ? "Заказ через интернет" : t_customer) + ((t_phone != "") ? ", телефон: " + t_phone : "") + " " + ((t_do != "") ? " Отправлено в печать: " + t_do : "") + ((t_dp != "") ? " Напечатано: " + t_dp : "") + ((t_dv != "") ? " Отправлено в центр выдачи: " + t_dv : "") + " СТАТУС: " + t_status + " (данные получены: " + DateTime.Now.ToShortDateString() + " " + DateTime.Now.ToLongTimeString() + ")\n";
			...
			if (!ok)
			{
				...
				if (File.Exists(prop.Dir_import + "//irorder.xml"))
				{
					FileInfo _f = new FileInfo(prop.Dir_import + "//irorder.xml");
					long _local_size = 0;
					DateTime _local_time = new DateTime();
					if (File.Exists(root_dir + "\\cache\\irorder.xml"))
					{
						FileInfo __f = new FileInfo(root_dir + "\\cache\\irorder.xml");
						_local_size = __f.Length;
						_local_time = __f.LastWriteTime;
					}
					if ((_f.Length != _local_size) || (_f.LastWriteTime != _local_time))
					{
						File.Copy(prop.Dir_import + "//irorder.xml", root_dir + "\\cache\\irorder.xml", true);
					}
				}
				if (File.Exists(root_dir + "\\cache\\irorder.xml"))
				{
					XmlTextReader xmlTextReader = new XmlTextReader(root_dir + "\\cache\\irorder.xml");
					xml.Load(xmlTextReader);
					xmlTextReader.Close();
					XmlNode root = xml.GetElementsByTagName("IRORDER")[0];
						...
						foreach (XmlNode node in onode.ChildNodes)
						{
							if (node.Name == "HEADER")
							{
								...
								if ((t_number == order) && (t_kiosk == kiosk))
								{
									...
									head.Add("PRINTED_AUTHDATE", t_dp);
									head.Add("SHIP_AUTHDATE", t_dv);
									m += "Заказ: " + t_number + ", принят " + t_stamp + " Клиент: " + ((t_customer == "") ? "Заказ через интернет" : t_customer) + ((t_phone != "") ? ", телефон: " + t_phone : "") + " " + ((t_do != "") ? " Отправлено в печать: " + t_do : "") + ((t_dp != "") ? " Напечатано: " + t_dp : "") + ((t_dv != "") ? " Отправлено в центр выдачи: " + t_dv : "") + " СТАТУС: " + t_status + " (данные получены: " + t_a + ")\n";
									found = true;
								}
			...
			if ((ok) && (m != "") && (od.Rows.Count != 0))
			{
				try
				{
					using (SqlConnection cn = new SqlConnection(prop.Connection_string))
					{
						...
						//Проверяем на наличие клиента с имененм "Заказ через интернет" и 9-ой категории прайса
						cmd = new SqlCommand("DECLARE @ID int; SET @ID = (SELECT [id_client] FROM [dbo].[client] WHERE RTRIM([name]) = 'Заказ через интернет'); IF(@ID > 0) BEGIN; SELECT @ID; END; ELSE BEGIN; INSERT INTO [dbo].[client] ([id_category], [id_dcard], [guid], [del], [name], [phone_1], [phone_2], [address], [email], [icq], [addon]) VALUES (9, 0, newid(), 0, 'Заказ через интернет', '', '', '', '', '', ''); SET @ID = scope_identity(); SELECT @ID; END;", cn);
						...
						cmd = new SqlCommand("SELECT * FROM [order] WHERE [number] = " + prop.Order_terminal_prefics.Substring(0, 1) + "1" + int.Parse(order).ToString("D10"), cn);
						...
						if (t_order.Rows.Count > 0)
						{
							...
							else
							{
								...
								{
									add = true;
									query += "UPDATE [order] SET \n\t[status] = '010000', \n\texported = 0, \n\t[number] = '" + prop.Order_terminal_prefics.Substring(0, 1) + "9" + int.Parse(order).ToString("D10") + "' \nWHERE \n\t[number] = " + prop.Order_terminal_prefics.Substring(0, 1) + "1" + int.Parse(order).ToString("D10") + ";\n";
						...
						if(add)
						{
							...
							if (prop.Terminal_client_one)
								query += "DECLARE @CLIENTID int;\n" +
									...
									"SET @CLIENTNAME = 'Заказ через интернет'\n";
							else
							...
							query += "INSERT INTO [dbo].[order]\n" +
							 ...
							 "VALUES \n" +
								   ...
								   ",'' \n" +
								   ",'000010' \n" +
								   ",'" + prop.Order_terminal_prefics.Substring(0, 1) + "1" +
							...
							query += "INSERT INTO [dbo].[kiosk_orders_ok] ([number], [status], [exported]) VALUES ('" + prop.Order_terminal_prefics.Substring(0, 1) + "1" + int.Parse(order).ToString("D10") + "', 1, 0);\n";
							query += "COMMIT;\n\n";

...
{
	public partial class frmPostTable : Form
	{
		...
		public frmPostTable(SqlConnection db_connection, UserInfo usr)
		{
			InitializeComponent();
			this.Text = "Должности";
		...
		private void frmPostTable_Load(object sender, EventArgs e)
		{
		...
		private void LoadData()
		{
			...
			db_command = new SqlCommand("SELECT [id_post], CAST([del] AS BIT) AS [del], [name] FROM [vwAdminPost] ORDER BY [name]", db_connection);
			...
			gridData.Cols[3].Caption = "Должность";
		...
		private void btnAdd_Click(object sender, EventArgs e)
		{
			try
			{
				frmEditPost f = new frmEditPost(db_connection, usr);
				f.ShowDialog();
		...
		private void btnEdit_Click(object sender, EventArgs e)
		{
			try
			{
				...
				{
					frmEditPost f = new frmEditPost(db_connection, usr, gridData.Rows[gridData.Row][1].ToString());
					f.ShowDialog();
					if (f.DialogResult == DialogResult.OK)
						LoadData();
					f.Close();
				...
				if (MessageBox.Show("Удалить эту запись?", "Удаление", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.OK)
				{
					SqlCommand c =
						new SqlCommand(
							...
							" WHERE id_post = " + gridData.Rows[gridData.Row][1].ToString(), db_connection);
					c.ExecuteNonQuery();
				}
			}
			catch (Exception ex)

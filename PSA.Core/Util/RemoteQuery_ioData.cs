...
namespace PSA.Lib.Util
{
	...
	{
		private void QueryData_io(string kiosk, string order)
		{
				...
				try
				{
					...
					foreach (XmlNode node in root.ChildNodes)
					{
						if (node.Name == "HEADER")
						{
							...
							head.Add("CUSTOMER", ((t_customer == "") ? "Оптовик через интернет" : t_customer));
							...
							m += "Заказ: " + t_number + ", принят " + t_stamp + " Клиент: " + ((t_customer == "") ? "Оптовик через интернет" : t_customer) + ((t_phone != "") ? ", телефон: " + t_phone : "") + " " + ((t_do != "") ? " Отправлено в печать: " + t_do : "") + ((t_dp != "") ? " Напечатано: " + t_dp : "") + ((t_dv != "") ? " Отправлено в центр выдачи: " + t_dv : "") + " СТАТУС: " + t_status + " (данные получены: " + DateTime.Now.ToShortDateString() + " " + DateTime.Now.ToLongTimeString() + ")\n";
						...
						foreach (XmlNode node in onode.ChildNodes)
						{
							if (node.Name == "HEADER")
							{
								...
								if ((t_number == order) && (t_kiosk == kiosk))
								{
									...
									head.Add("ADDRESS", t_address);
									head.Add("STAMP", t_stamp);
									head.Add("CUSTOMER", ((t_customer == "") ? "Оптовик через интернета" : t_customer));
									head.Add("PHONE", t_phone);
									head.Add("STATUS", t_status);
									head.Add("TOPRINT_AUTHDATE", t_do);
									head.Add("PRINTED_AUTHDATE", t_dp);
									head.Add("SHIP_AUTHDATE", t_dv);
									m += "Заказ: " + t_number + ", принят " + t_stamp + " Клиент: " + ((t_customer == "") ? "Оптовик через интернет" : t_customer) + ((t_phone != "") ? ", телефон: " + t_phone : "") + " " + ((t_do != "") ? " Отправлено в печать: " + t_do : "") + ((t_dp != "") ? " Напечатано: " + t_dp : "") + ((t_dv != "") ? " Отправлено в центр выдачи: " + t_dv : "") + " СТАТУС: " + t_status + " (данные получены: " + t_a + ")\n";
									found = true;
								}
			...
			if ((ok) && (m != "") && (od.Rows.Count != 0))
			{
				try
				{
					using (SqlConnection cn = new SqlConnection(prop.Connection_string))
					{
						...
						//Проверяем на наличие клиента с имененм "Оптовик через интернет" и 9-ой категории прайса
						cmd = new SqlCommand("DECLARE @ID int; SET @ID = (SELECT [id_client] FROM [dbo].[client] WHERE RTRIM([name]) = 'Оптовик через интернет'); IF(@ID > 0) BEGIN; SELECT @ID; END; ELSE BEGIN; INSERT INTO [dbo].[client] ([id_category], [id_dcard], [guid], [del], [name], [phone_1], [phone_2], [address], [email], [icq], [addon]) VALUES (9, 0, newid(), 0, 'Оптовик через интернет', '', '', '', '', '', ''); SET @ID = scope_identity(); SELECT @ID; END;", cn);
						...
						if(add)
						{
							...
							if (prop.Terminal_client_one)
								query += "DECLARE @CLIENTID int;\n" +
									...
									"SET @CLIENTNAME = 'Оптовик через интернет'\n";
							else
							...
							query += "INSERT INTO [dbo].[order]\n" +
							 ...
							 "VALUES \n" +
								   ...
								   ",'' \n" +
								   ",'000010' \n" +
								   ",'" + prop.Order_terminal_prefics.Substring(0, 1) + "2" +

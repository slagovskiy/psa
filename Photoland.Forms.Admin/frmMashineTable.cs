...
{
	public partial class frmMashineTable : Form
	{
		...
		public frmMashineTable(SqlConnection db_connection, UserInfo usr)
		{
			InitializeComponent();
			this.Text = "Машины";
		...
		private void frmMashineTable_Load(object sender, EventArgs e)
		{
		...
		private void LoadData()
		{
			...
			db_command = new SqlCommand("SELECT [id_mashine], CAST([del] AS BIT) AS [del], [mashine] FROM [vwAdminMashine] ORDER BY [mashine]", db_connection);
			...
			gridData.Cols[3].Caption = "Машина";
		...
		private void btnAdd_Click(object sender, EventArgs e)
		{
			try
			{
				frmEditMashine f = new frmEditMashine(db_connection, usr);
				f.ShowDialog();
		...
		private void btnEdit_Click(object sender, EventArgs e)
		{
			try
			{
				...
				{
					frmEditMashine f = new frmEditMashine(db_connection, usr, gridData.Rows[gridData.Row][1].ToString());
					f.ShowDialog();
					if (f.DialogResult == DialogResult.OK)
						LoadData();
					f.Close();
		...
		private void btnDelete_Click(object sender, EventArgs e)
		{
			try
			{
				if (MessageBox.Show("Удалить эту запись?", "Удаление", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.OK)
				{
					SqlCommand c =
						new SqlCommand(
							"UPDATE [mashine] SET [del] = 1" +
							" WHERE [id_mashine] = '" + gridData.Rows[gridData.Row][1].ToString() + "'", db_connection);
					c.ExecuteNonQuery();
				}
			}
			catch (Exception ex)

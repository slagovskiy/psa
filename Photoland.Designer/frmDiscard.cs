...
namespace Photoland.Designer
{
	public partial class frmDiscard : Form
	{
		...
		{
			SqlCommand m = new SqlCommand("SELECT [id_material], [material] FROM [vwMaterialListDesigner] ORDER BY [material]", db_connection);
				...
				if (this._id == 0)
				{
					if ((decimal.Parse(txtQuantity.Text) != 0) && (txtMaterial.SelectedValue.ToString() != "0"))
					{
						try
						{
							...
							SqlCommand t =
								new SqlCommand(
									"INSERT INTO [discard] ([del], [guid], [datediscard], [id_material], [quantity], [comment], [id_user], [user_name], [orderno]) VALUES (0,'" +
